...
% Modificada y corregida por Carlos Félix Pardo Martín.
...
\url{https://github.com/picuino/ThinkPython2}
...
Desde entonces he continuado desarrollando el libro,
...
\item Agregué más ejercicios, que van desde pruebas cortas de comprensión
  hasta algunos proyectos sustanciales.  La mayoría de los ejercicios
...
muchos errores tipográficos en el Capítulo 3.  Además, encontró errores en 
la función {\tt increment} en el Capítulo 13.
...
del libro, y han hecho numerosas correcciones y sugerencias.

\item Keith Verheyden envió una corrección en el Capítulo 3.
...
\item Ivo Wever encontró un error tipográfico en el Capítulo 5 y sugirió
una aclaración en el Capítulo 3.
...
\item Nam Nguyen encontró un error tipográfico y advirtió que usé el patrón 
Decorator pero sin mencionar el nombre.
...
\item Jorge Espinoza es el autor de esta traducción del libro al castellano.

\item Carlos Félix Pardo Martín ha realizado numerosas correcciones y
modificaciones menores al libro traducido al castellano.
La lista completa de modificaciones puede leerse en
\url{https://github.com/picuino/ThinkPython2/blob/master/README.md#modificaciones}

% ENDCONTRIB

Adicionalmente, las personas que vieron errores tipográficos o hicieron
correcciones incluyen a Czeslaw Czapla, Dale Wilson, Francesco Carlo Cimini,
...
componentes en sistemas y evaluando comparaciones entre alternativas.
...
diferente para ti.  Sin embargo, deberías verificar que el número de versión, que
...
El guion bajo, \verb"_", puede aparecer en un nombre.
...
Un valor por sí mismo es considerado una expresión, y por consiguiente lo es
...
\item Los {\bf E}xponentes de potencias tienen la siguiente prioridad, así que
...
de $\pi$, con precisión de alrededor de 15 dígitos.
\index{pi}
...
los números y el guion bajo son legales, pero el primer carácter
...
programa que dibuja cuadrados.  No continues hasta que lo hagas.
...
refactorizarlo dentro de una función general apropiada.
...
triángulo rectángulo, dadas las longitudes de los otros dos lados como argumentos.
...
\end{verbatim}
%
Esto significa ``obtén el valor actual de {\tt x}, suma uno, y luego
...
{\tt sort} ordena los elementos de la lista de menor a mayor:
...
\end{verbatim}
%
Nuevamente, las claves no están en un orden particular.  Para recorrer las claves
...
de tu programa se verá afectado.
...
Esta es una solución ineficiente para el problema y se pone peor
...
\index{variable global!actualizar}
...
Aquí, las tuplas se muestran utilizando la notación de la sintaxis de Python.
...
valor aleatorio del histograma, escogido con probabilidad
...
podemos utilizar \verb"procesar_archivo" para construir un histograma para
...
las operaciones que necesitarás implementar para cada estructura de datos.
...
es fácil; escoger del histograma es más difícil de hacer 
...
Una última reflexión: en esta discusión, he insinuado que
...
no pueden soportar eliminar una línea de código (incluso si es incorrecta).
...
log $f$ y log $r$.  Utiliza el programa de gráficos de tu
...
En una gran colección de archivos MP3, puede haber más de una
...
se refiere y obtén el valor de {\tt x}.''  En este ejemplo, asignamos ese
...
``Ve al objeto al cual {\tt caja} se refiere y selecciona el atributo con nombre
...
objeto Tiempo y lo imprima en la forma {\tt hora:minuto:segundo}.
...
Su nombre completo es \verb"__init__" (dos caracteres de guion bajo,
...
Cada vez que tengas inseguridad acerca del flujo de ejecución a través de tu
...
Luego, transformamos las funciones en métodos.  Por ejemplo,
...
\end{verbatim}
%
El operador \verb"<=" verifica si un conjunto es subconjunto de otro,
...
diccionario que mapea de una cadena de letras ordenada a la lista de
...
que veas algo similar, serás capaz de encontrar el error con más rapidez.
...
respondió: ``Pienso que el ordenamiento de burbuja sería la manera incorrecta de proceder.''
...
si sé que el tiempo de ejecución del Algoritmo A tiende a ser
...
Para los términos logarítmicos, la base del logaritmo no importa:
...
\item ¿Qué algoritmo de ordenamiento utiliza la librería de C?  ¿Qué algoritmo de ordenamiento
  utiliza Python?  ¿Son estos algoritmos estables?  Quizás tengas que
