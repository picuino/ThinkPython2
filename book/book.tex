...
\end{verbatim}
%
Si no estás seguro de qué tipo es un objeto, puedes consultar:
\index{función type}
\index{type!función}
...
\end{verbatim}
%
Si no estás seguro de si un objeto tiene un atributo particular,
...
\label{increment}
\index{modificador}
\index{tipo de función!modificador}
...
Una vez que estés convencido de que están correctas, puedes utilizarlas para
...
los programas de los últimos dos capítulos no son realmente
orientados a objetos debido a que no representan las relaciones
entre los tipos definidos por el programados y las funciones que operan
en estos.  El siguiente paso es transformar esas funciones en
métodos que hagan explícitas a las relaciones.
...
funciones que definimos a los tipos de cosas que las personas hacen con
los tiempos.  De manera similar, las clases {\tt Punto} y {\tt Rectángulo}
...
no son estrictamente necesarias; la mayoría de estas proporcionan
una sintaxis alternativa para cosas que ya hemos hecho.  Sin embargo, en muchos casos,
la alternativa es más concisa y comunica de manera más precisa
la estruvtura del programa.

Por ejemplo, en {\tt Time1.py} no hay conexión
obvia entre la definición de clase y las definiciones de función
que siguen.  Con algo de revisión, es aparente que cada función
...
\item La sintaxis para invocar un método es diferente a la
...
mover la definición de funcióin al interior de la definición de clase.  Notar
...
e \verb"imprimir_tiempo" es el nombre del método.  {\tt comienzo} es
...
el método, el cual se llama {\bf sujeto}.  Tal como el
...
\index{metáfora, invocación a método}
...
responsable hace posible escribir funciones más versátiles (o
métodos), y hace más fácil mantener y reutilizar código.
...
este caso es convencional ponerle el nombre {\tt self} al primer parámetro
...
un método especial que es invocado cuando se instancia un objeto.
...
\index{método str@método \_\_str\_\_}
...
operador en Python hay un método especial que le corresponde, como
\verb"__add__".  Para más detalles, ver
...
Si no estás seguro de si un objeto tiene un atributo en particular,
...
interfaces separadas de las implementaciones.  Para objetos, eso significa
...
Sin embargo, si diseñaste la interfaz concuidado, puedes
...
\index{lenguage orientado a objetos}
...
\item[sujeto:] El objeto en el cual se invoca un método.
\index{sujeto}
...
de un operando e invoca funciones diferentes para tipos diferentes.
\index{despacho basado en tipo}
...
más comunes, y más difíciles de encontrar, en Python.
Escribe una definición de una clase con nombre {\tt Canguro} con los siguientes
métodos:

\begin{enumerate}

\item Un método \verb"__init__" que inicialice un atributo con nombre
\verb"pouch_contents" a una lista vacía.

\item Un método con nombre \verb"put_in_pouch" que tome un objeto
de cualquier tipo y lo añada a \verb"pouch_contents".

\item Un método \verb"__str__" que devuelva una representación de cadena
del objeto Canguro y los contenidos de la bolsa.
...
\url{http://thinkpython2.com/code/GoodKangaroo.py}, el cual explica el
