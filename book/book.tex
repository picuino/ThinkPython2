...
\item John Ouzts corrigió la definición de ``return value''
en el Capítulo 3.
...
de programación se llama ``Hello, World!'' porque todo lo que hace es mostrar
las palabras ``Hello, World!''.  En Python, se ve así:
...
Después de ``Hello, World'', el siguiente paso es la aritmética.  Python
...
a una nueva variable llamada {\tt message};
...
\verb"your_name" o \verb"airspeed_of_unladen_swallow".
\index{guión bajo}
...
La primera línea asigna un valor a {\tt miles}, pero no tiene un efecto
...
\item Los {\bf E}xponentes (potencias) tienen la siguiente proridad, así que
...
se llama el {\bf argumento} de la función.  El resultado, para esta
...
Python tiene un módulo math que proporciona la mayor parte de las
funciones matemáticas familiares.  Un {\bf módulo} es un archivo que contiene una
...
del módulo y el nombre de la función, separado por un punto.
Este formato se llama {\bf notación punto}.
...
una relación señal-ruido en decibeles (suponiendo que \verb"signal_power" y
\verb"noise_power" están definidas).  El módulo math también proporciona {\tt log},
...
El segundo ejemplo encuentra el seno de {\tt radians}.  El nombre de la variable {\tt radians} es un indicio de que {\tt sin} y las otras funciones
...
la raíz cuadrada de dos, dividido por dos:
\index{función sqrt}
\index{sqrt!función}
...
el resto se llama {\bf cuerpo} (en inglés, {\em body}).  El encabezado tiene que terminar con el signo de dos puntos
y el cuerpo tiene que tener sangría.  Por convención, la sangría es
siempre cuatro espacios.  El cuerpo puede contener
...
ubicadas al lado de Entrar en el teclado.  ``Comillas tipográficas'', como
...
\end{verbatim}
%
Y luego llamar a \verb"repeat_lyrics":
...
\end{verbatim}
%
Este programa contiene dos definiciones de función: \verb"print_lyrics" y
\verb"repeat_lyrics".  Las definiciones de funciones se ejecutan al igual que otras
...
y mueve la definición de \verb"print_lyrics" a después de la definición de
\verb"repeat_lyrics".  ¿Qué ocurre cuando ejecutas este programa?


\section{Flujo de ejecución}
\index{flujo de ejecución}

Para asegurar que una función está definida antes de su primer uso,
...
como un argumento.  Algunas funciones toman más de un argumento:
...
con nombre {\tt bruce}.  Cuando la función es llamada, imprime el valor del
...
como un argumento para \verb"print_twice":
\index{composición}
...
\end{verbatim}
%
El nombre de la variable que pasamos como un argumento ({\tt michael}) no tiene
nada que ver con el nombre del parámetro ({\tt bruce}).  No
importa cómo se le llame al valor en su casa (en la función que llama);
aquí en \verb"print_twice", a todos les llamamos {\tt bruce}.
...
\end{verbatim}
%
Cuando \verb"cat_twice" termina, la variable {\tt cat}
...
Por ejemplo, afuera de \verb"print_twice", no hay
...
Cada función se representa por un {\bf marco}.  Un marco es un recuadro con
el nombre de una función junto a éste y los parámetros y variables de
la función dentro de éste.  El diagrama de pila para el ejemplo anterior se
...
llama a cuál, y así.  En este ejemplo, \verb"print_twice"
fue llamado por \verb"cat_twice", y \verb"cat_twice" fue llamado por
\verb"__main__", el cual es un nombre especial para el marco más alto.  Cuando
creas una variable afuera de cualquier función, pertenece a
\verb"__main__".

\index{main}

Cada parámetro hace referencia al mismo valor que su argumento
correspondiente.  Así que, {\tt part1} tiene el mismo valor que
{\tt line1}, {\tt part2} tiene el mismo valor que {\tt line2}
...
\verb"print_twice", obtienes un {\tt NameError}:
...
Puede no estar claro por qué vale la pena el problema de dividir
...
\item Las funciones pueden hacer un programa más pequeño eliminando código
...
Escribe una función con nombre \verb"right_justify" que tome una cadena
...
pasarlo como argumento.  Por ejemplo, \verb"do_twice" es una función
...
Aquí hay un ejemplo que usa \verb"do_twice" para llamar a una función
con nombre \verb"print_spam" dos veces.
...
\item Modifica \verb"do_twice" para que tome dos argumentos, un
objeto de función y un valor, y llame a la función dos veces,
pasando al valor como argumento.

\item Copia la definición de
\verb"print_twice", presentada previamente en este capítulo, a tu script.

\item Usa la versión modificada de \verb"do_twice" para llamar a
\verb"print_twice" dos veces, pasando a \verb"'spam'" como argumento.

\item Define una nueva función llamada
\verb"do_four" que tome un objeto de función y un valor
...
anular ese comportamiento y poner un espacio al final, como esto:
