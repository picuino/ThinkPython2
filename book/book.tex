...
El operados {\bf división entera}, \verb"//", divide
...
{\tt x > 0 and x < 10} es verdadera sólo si {\tt x} es mayor que 0
{\em y} menor que 10.
\index{operador and}
\index{operador or}
\index{operador not}
...
que podrían ser confusas.  Podrías querer evitarla (a menos que
sepas lo que estás haciendo).


\section{Ejecucioń condicional}
...
Para escribir programas útiles, casi necesitamos la capacidad
de verificar las condiciones y cambiar el comportamiento del programa
como corresponde.  Las {\bf sentencias condicionales} nos dan esta habilidad.  La
forma más simple es la sentencia {tt if}:
...
llama {\bf condición}.  Si ésta es verdadera, se ejecutan las sentencias
...
termina.  Incluso si más de una condición es verdadera, sólo se ejecuta
...
\end{verbatim}
%
La sentencia {\tt print} se ejecuta sólo si pasamos por los dos
...
La {\tt cuenta\_reg} que obtuvo {\tt n=1} termina.
\end{quote}

La {\tt cuenta\_reg} que obtuvo {\tt n=2} termina.
\end{quote}

La {\tt cuenta\_reg} que obtuvo {\tt n=3} termina.
...
\end{verbatim}
%
Si {\tt n <= 0} la {\bf sentencia return} hace que salga de laa función.  El
...
marco que contiene las variables locales y parámetros de la función.
Para una función revursiva, podría haber más de un marco en la
...
\verb"__main__" ni pasamos argumentos en ésta.
\index{caso base}
...
\end{verbatim}
%
La secuencia \verb"\n" al final de la pregunta representa una {\bf
  nueva línea}, el cual es un carácter especial que provoca un salto de línea.
Esa es la razón por la cual la entrada del usuario aparece debajo de la pregunta.  \index{nueva línea}

Si esperas que el usuario escriba un entero, puedes intentar convertir
el valor de retorno a {\tt int}:

\begin{verbatim}
>>> pregunta = '¿Cuál...es la velocidad media de una golondrina sin carga?\n'
...
mucha información, pero puede ser abrumadora.  Las partes
...
\item[división entera:] Un operador, denotado por {\tt //}, que divide dos
  números y redondea a un entero hacia abajo (hacia el infinito negativo).
  \index{división entera}
  \index{entera!división}
...
\item[operador relacional:] Una de los operadores que comprara
...
enteros y use a \verb"comprobar_fermat" para comprobar si éstos
...
capaz de hacer que los palos se encuentren en el medio.  Para tres
