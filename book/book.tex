...
Ahora modifica el programa para dibujar un cuadrado.  ¡No avances hasta que
...
Agrega este ejemplo a {\tt mypolygon.py} y ejecútalo de nuevo:
\index{bucle for}
\index{for!bucle}
\index{sentencia for}
...
programa que dibuja cruadrados.  No avances hasta que lo hagas.
...
Una sentencia {\tt for} tambiés es llamada {\bf bucle} porque
...
Escribe una llamada a función que pase a {\tt bob} como un argumento a 
{\tt cuadrado}, y luego ejecuta el programa de nuevo.

\item Agrega otro parámetro, con nombre {\tt largo}, a {\tt cuadrado}.
Modifica el cuerpo para que el largo de los lados sea {\tt largo}, y luego
...
{\tt t}, y radio, {\tt r}, como parámetros y que dibuje un círculo
aproximado llamando a {\tt polygon} con un largo y
número de lados apropiado.  Prueba tu función con un rango de valores
de {\tt r}.  \index{funcíon círculo} \index{círculo!función}

Pista: averigua el perímetro del círculo y asegúrate de que
{\tt largo * n = perimetro}.
...
El primer ejercicio te pide poner tu código dibuja cuadrados
dentro de una definición de función y luego llamar a la función, pasando
a la tortuga como un parámetro.  Aquí está la solución:
...
tortuga, no sólo bob {\tt bob}, así que podrías crear una segunda tortuga y
...
El siguiente paso es agregar un parámetro {\tt largo} a {\tt cuadrado}.
Aquí hay una solución:

\begin{verbatim}
def cuadrado(t, largo):
    for i in range(4):
        t.fd(largo)
        t.lt(90)
...
\end{verbatim}
%
Este ejemplo dibuja un polígono de 7 lados de largo 70.
