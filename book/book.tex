...
Un tipo definido por el programados también se llama {\bf clase}.
...
Puedes asignar valores a una instancia utilizando notacíon de punto:
...
Un diagrama de estado que muestra un objeto y sus atributos se
...
{\tt altura} con números; {\tt esquina} es un objeto Punto que
...
sus atritubos.  Por ejemplo, para cambiar el tamaño de un rectángulo
...
{\tt d\_anchura} and {\tt d\_altura}, and adds the numbers to the
width and altura of the rectangle:
...
\end{verbatim}
%
Dento de la función, {\tt rect} es un
...
Los alias puedes hacer que un programa sea difícil de leer porque los cambios
...
\end{verbatim}
%
El operados {\tt is} indica que {\tt p1} y {\tt p2} no son el
mismo objeto, que es lo que esperábamos.  Sin embargo, tal vez hayas esperado
que {\tt ==} entregue {\tt True} porque estos pountos contienen los mismos
...
de objeto.  Eso ocurre debido a que para tipos definidos por el programados, Python no
...
y así sucesivamente.
No te sorporenderá aprender que esta operación se
...
\item[class object:] Un objeto que contiene información acerca del
...
\item[objeto incrustado:] Uno objeto que se almacena como atributo
de otro objeto.
\index{objeto incrustado}
\index{incrustado!objeto}

\item[copia superficial:] Copiar los contenidos de un objeto, incluyendo
cualquier referencia a objetos incrustados;
