...
definir una clase nuevo que sea una verión modificada de una clase existente.
...
juego que estés jugando, un As puede tener más valor que un Rey
o menor que 2.
\index{rango}
\index{palo}

Si queremos definir un objeto nuevo que represente una carta, es
obvio cuáles deberían ser los atributos: {\tt rango} y
{\tt palo}.  No es obvio qué tipo de atributos
deberían ser.  Una posibilidad es utilizar cadenas que contengan palabras como
\verb"'Picas'" para palos y \verb"'Reinga'" para rangos.  Un problema con
esta implementación es que no sería fácil comparar cartas para
...
A ambos tipos de atributo se accede utilizando notación de punto.  Por
...
Poniéndolos todos juntos, la expresión
...
de la clase {\tt Carta}, y selecciona la cadena apropiada.''
...
y así sucesivamente.
\index{método cmp@método \_\_cmp\_\_}
...
\index{método join} \index{join!método} \index{nueva línea}
...
\index{método sort} \index{sort!método}
...
con 52 cartas nuevas, el método init para Manos debería inicializar {\tt
  cartas} con una lista vacía.  \index{anular} \index{método init}
\index{init!método}

Si proporcionamos un método init en la clase {\tt Mano}, este lo anula
...
En algunos juegos, las cartas de mueven de una mano a otra,
...
hacer tan bien o mejor sin esta.
...
Supongamos que estás escribiendo una función que trabaje con objetos Mano.
...
entre el objeto y alguna entidad del mundo real (o al menos un
mundo matemático).
...
En análisis de Markov, de la Sección~\ref{markov}, proporciona un buen ejemplo.
...
\item En capsula variables relacionadas como atributos de un objeto.
...
versión odificada de una clase definida anteriormente.
\index{herencia}
...
\index{clase hija}
\index{hija!clase!child}

\item[relación IS-A:] Una relación entre una clase hija
y su clase padre.
\index{relación IS-A}

\item[relación HAS-A:] A relationship between two classes
...
\item Imprime una tabla de las clasificaciones y sus probabilidades.
