...
{\em Pensar en Python}.  Algunos de los cambios son:
...
se verifica la siguiente, y así.  Si una de ellas es
...
haga referencia a un nuevo valor (y deje de hacer referencia al valor antiguo).
...
Cada vez que se pasa por el bucle, el programa muestra el valor de {\tt n}
...
letra, {\tt fruta[-2]} entrega la penúltima, y así.
...
\end{verbatim}
%
Cada vez que se pasa por el bucle, el siguiente carácter en la cadena se asigna
...
\label{slice}
\index{operador de trozo} \index{trozo!operador de} \index{índice de trozo}
...
\section{Bucle y conteo}
\label{counter}
\index{contador}
\index{conteo y bucle}
\index{bucle y conteo}
...
\end{verbatim}
%
La primera vez que se pasa por el bucle, el valor de {\tt j} es 4,
...
\item[objeto:] Algo a lo cual una variable puede hacer referencia.  Por ahora,
...
Un tamaño de paso de -1 pasa a través de la palabra hacia atrás, por lo cual
...
\end{verbatim}
%
Este bucle comienza con {\tt i=0} y termina cuando {\tt i=len(palabra)-1}.  Cada
vez que se pasa por el bucle, compara al $i$-ésimo carácter (que puedes
...
y los elementos de la lista por dentro.  {\tt quesos} hace referencia a
...
{\tt vacio} hace referencia a una lista sin elementos.
\index{asignación de ítem}
\index{ítem!asignación de}
\index{reasignación}
...
\index{lista!membership}
\index{membership!lista}
\index{operador in}
\index{in!operador}
...
la lista.  Cada vez que se pasa por el bucle {\tt i} obtiene el índice
...
\section{Mapeo, filtro y reducción}
\label{filter}
...
\end{verbatim}
%
{\tt total} se inicializa en 0.  Cada vez que se pasa por el bucle,
...
elementos; una variable que se usa de esta manera a veces se llama
{\bf acumulador}.
...
\end{verbatim}
%
{\tt res} se inicializa con una lista vacía; cada vez que se pasa por el bucle,
anexamos el elemento siguiente.  Entonces {\tt res} es otro
tipo de acumulador.
\index{acumulador lista}

Una operación como \verb"todas_con_mayuscula" a veces se llama {\bf
mapeo} porque ``mapea'' una función (en este caso el método {\tt
capitalize}) sobre cada uno de los elementos en una secuencia.
\index{patrón de mapeo}
\index{mapeo!patrón de}
...
cadena.  Para convertir de una cadena a una lista de caracteres,
puedes usar {\tt list}:
\index{list!función}
\index{función!list}
...
evitar usarlo como nombre de variable.  También evito {\tt l} porque
...
\end{verbatim}
%
Sabemos que {\tt a} y {\tt b} hacen referencia a una
cadena, pero no
sabemos si hacen referencia a la {\em misma} cadena.
...
En el primer caso, {\tt a} y {\tt b} hacen referencia a dos objetos diferentes que
tienen el mismo valor.  En el segundo caso, hacen referencia al mismo
objeto.
\index{operador is}
\index{is!operador}

Para verificar si dos variables hacen referencia al mismo objeto, puedes
...
En este ejemplo, Python solo crea un objeto de cadena y tanto {\tt
  a} como {\tt b} hacen referencia a este.  Pero cuando creas dos listas, obtienes
dos objetos:
...
{\tt x} es un elemento de lista, estos son correctos:
...
\item[equivalente:] Tener el mismo valor.
\index{equivalente}

\item[idéntico:] Ser el mismo objeto (lo cual implica equivalencia).
\index{idéntico}
